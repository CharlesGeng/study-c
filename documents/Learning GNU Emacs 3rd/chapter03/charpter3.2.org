...
*3.2.5 Emacs查询时是大小写敏感的吗？*

默认情况下，Emacs查询时不区分大小写。查看选项菜单，你会发现 *Case-Insensitive* 选项是唯一一个默认选中的选项。

在实际使用中，这样做有什么意义呢？如果你查找单词 *randon* , 会找到 *random* , *Random* 和 *RANDOM* ，甚至像 *RanDoM* 和 *rANdOM*
这样怪异的匹配项也能找到。Emacs在替换时会注意要被替换的单词并且按照原词的大小写样式替换。如果你想将 /random/ 替换成 /tandem/ ,那
么 /Random/ 会被替换成 /Tandem/ ， /RANDOM/ 替换成 /TANDEM/ 。如果你按大小写混合输入替换的字符串，那么替换操作会按照你输入的格式替换。
当输入的字符串为大小写区分的方式输入HealthBug，那么 /healthbug/ 会被替换为 /HealthBug/ 。换言之，查找替换操作默认情况下一般按照
以下方式工作：Emacs查找字符串时不区分大小写并根据上下文适当地执行替换操作。不过，有时你需要更好的控制。
...
变量 *case-fold-search* 决定查询时是否大小写敏感。它适用于所有的查询操作：递增查询，单词查询，以及查询替换操作中的查询等等。默认
情况下，变量 *case-fold-search* 被设置为 *t* , *t* 表示“除非用户按大小写混合输入或者全部大写，否则不区分大小写”。这个明智的默
认项与用户一般要求相符。不过，如果需要大小写敏感，菜单中
